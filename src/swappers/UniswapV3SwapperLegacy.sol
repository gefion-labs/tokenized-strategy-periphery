...
import {ISwapRouterLegacy} from "../interfaces/Uniswap/V3/ISwapRouterLegacy.sol";
 ...
 */
contract UniswapV3SwapperLegacy {
    ...
    function _swapFrom(
        ...
        if (_amountIn > minAmountToSell) {
            ...
            if (_from == base || _to == base) {
                ISwapRouterLegacy.ExactInputSingleParams memory params = ISwapRouterLegacy
                ...
                _amountOut = ISwapRouterLegacy(router).exactInputSingle(params);
            } else {
                ...
                _amountOut = ISwapRouterLegacy(router).exactInput(
                    ISwapRouterLegacy.ExactInputParams(
                        path,
                        address(this),
                        block.timestamp,
                        _amountIn,
    ...
    function _swapTo(
        ...
        if (_maxAmountFrom > minAmountToSell) {
            ...
            if (_from == base || _to == base) {
                ISwapRouterLegacy.ExactOutputSingleParams memory params = ISwapRouterLegacy
                ...
                _amountIn = ISwapRouterLegacy(router).exactOutputSingle(params);
            } else {
                bytes memory path = abi.encodePacked(
                    _to,
                    uniFees[base][_to], // base-to fee
                ...
                _amountIn = ISwapRouterLegacy(router).exactOutput(
                    ISwapRouterLegacy.ExactOutputParams(
                        path,
                        address(this),
                        block.timestamp,
